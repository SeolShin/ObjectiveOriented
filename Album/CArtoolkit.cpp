...
int CArtoolkit::Capture()
{
	//glutInit(2, "test");
    if( this->init() < 0 ) exit(0);
...
//    }
    ...
    /* detect the markers in the video frame */
		int cccccccccccccccc =  arDetectMarker(dataPtr, AR->thresh, &marker_info, &marker_num);
    ...
    for( j = 0; j < marker_num; j++ ) {
		if( marker_info[j].id == AR->target_id ) {
            if( k == -1 ) k = j;
            else {
    ...
    if( k != -1 ) {
        ...
        switch( AR->outputMode ) {
            case 0:
                AR->getResultRaw( &marker_info[k] );
                break;
            case 1:
				AR->getResultQuat( &marker_info[k] );
                break;
        }
    }
...
void CArtoolkit:: getResultRaw( ARMarkerInfo *marker_info )
{
...
void CArtoolkit::getResultQuat( ARMarkerInfo *marker_info )
{
    double      target_trans[3][4];
    double      cam_trans[3][4];
    double      quat[4], pos[3];
...
int CArtoolkit::init(void)
{
    ...
    /* open the video path */
	int aaaaaaaaaaaaaaa = arVideoOpen( vconf );
    if( aaaaaaaaaaaaaaa < 0 ) exit(0);
    /* find the size of the window */
    if( arVideoInqSize(&xsize, &ysize) < 0 ) exit(0);
    printf("Image size (x,y) = (%d,%d)\n", xsize, ysize);

    /* set the initial camera parameters */
	int bbbbbbbbbbbbbbbbbbbbbbbb = arParamLoad(cparaname, 1, &wparam);
    if( bbbbbbbbbbbbbbbbbbbbbbbb < 0 ) {
...
void CArtoolkit::keyEvent( unsigned char key, int x, int y)
{
...
void CArtoolkit:: motionEvent( int x, int y )
{
    if( mouse_st == 1 ) {
...
void CArtoolkit::mouseEvent(int button, int state, int x, int y)
{
    if( state == GLUT_UP ) {
        mouse_st = 0;
    }
